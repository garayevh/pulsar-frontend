...
export default function TestCasesPage() {
  ...
  const testCases = (session?.testCases ?? []) as any[]
  ...
  const handleApprove = async () => {
    setApproving(true)
    try {
      await analysisApi.reviewTestCases(sessionId, true)
      setStage('export')
    } catch (e) {
      console.error(e)
    } finally {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      a.href = url
      a.download = `test-cases-${sessionId.slice(0, 8)}.txt`
      a.click()
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex-1 overflow-y-auto p-8">
        <div className="mb-6">
          <h1 className="text-2xl font-semibold text-surface-900">Test Cases</h1>
      ...
      <div className="sticky bottom-0 border-t border-surface-200 bg-white p-4 flex gap-3">
        ...
        {session?.currentStage === 'export' && (
          <button
            ...
            disabled={exporting}
            className="flex items-center gap-2 rounded-lg border border-surface-200 px-6 py-2.5 text-sm font-medium text-surface-700 hover:bg-surface-50 disabled:opacity-50"
          >
